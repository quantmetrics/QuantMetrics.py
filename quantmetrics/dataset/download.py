...
The folloing codes are coied from download.py of chainer.
https://github.com/chainer/chainer/blob/master/chainer/dataset/download.py
"""
...
def set_dataset_root(path):
    ...
    environment variable ``QUANTMETRICS_DATASET_ROOT``. The other is by using this
    function. If both are specified, one specified via this function is used.
    The default dataset root is ``$HOME/.quantmetrics/dataset``.
    Args:
